---
type: Domain Concept
title: ChangesetGrouping
description: G1–G7 configurable groupings of HistoryEntry (v3.10). G1 per-unit, G2 per-battle, G3 per-team, G4 per-campaign, G5 per-supplement, G6 cross-campaign, G7 custom.
resource: "https://github.com/kaykayyali/crusade-master-prds/blob/a916708/prd-4-events-deltas.md"
tags:
  - concept
  - grouping
  - v3.10
timestamp: "2026-06-28T23:28:00Z"
---

# ChangesetGrouping




# Cross-references

- [HistoryEntry](/concepts/history-entry.md)
- [PRD-4](/prds/prd-4-events-deltas.md)
- [Rollback](/concepts/rollback.md)
