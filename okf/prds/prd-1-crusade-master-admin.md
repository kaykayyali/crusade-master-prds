...
Per [PRD-0](/prds/prd-0-overview.md) §3b. The roles in this app:
...
**Terminology:** the wizard uses "Step" (UI navigation). Don't conflate with **CampaignPhase** ([PRD-4](/prds/prd-4-events-deltas.md) §3.2), which is a CM-authored narrative period within a `started` campaign. The wizard steps exist only during campaign creation and aren't persisted; phases are persistent and affect the live campaign.
...
The CM creates the campaign's teams. For Armageddon, the system pre-fills the 4 template teams (Helsreach Defenders, Hades Defenders, Gorgutz's WAAAGH!, Skari's Kult) with their `expectedFactionIds` seeded from the book ([PRD-1](/prds/prd-1-crusade-master-admin.md) §5b). The CM can use as-is, edit, delete, or add custom teams.
...
**Step 3: Rules** — enable/disable `RulePack`s per campaign ([PRD-3](/prds/prd-3-army-export-versioning.md) §6). Default packs: `builtin-core`, `armageddon-narrative`. CM can disable packs they don't want.

**Step 4: Approvals** — per-kind team-leader authority ([PRD-1](/prds/prd-1-crusade-master-admin.md) §4.4 defaults), `teamLeaderApprovalMode: 'any' | 'all'` (default `'any'`), bulk-approve cap (default 50), `auto_approve_routine_battle_updates` (default false), `require_battle_report` (default true).
...
**Step 6: Review & Start** — summary of all settings. CM clicks "Start campaign" to flip `Campaign.status: 'created' → 'started'` (per [PRD-0](/prds/prd-0-overview.md) §4 v3.18 enum: `created` → `started` → `ended` → `archived`). **Hard gate:** if any team still has no team leader (the pending invites haven't been accepted), the system blocks Start with: "Team X has no team leader. Either assign one or wait for invites to be accepted."
...
- **Crusade Team Leader management (CM-only, per v3.12):**
  ...
  - A team can have **multiple team leaders** (v3.12). The CM promotes a player to team leader by adding them to `TeamLeader` join table ([PRD-0](/prds/prd-0-overview.md) §4) with `grantedAt` and `grantedByUserId`. The player must be a member of that team (via `CampaignMember`).
...
- **Team leader removal workflow (v3.12):**
  ...
  - The removed team leader loses the `crusade_team_leader` role for that team immediately. Their in-flight approvals (if any) are reassigned: pending approvals where they were the sole reviewer fall back to the primary CM (auto-reroute per [PRD-5](/prds/prd-5-approval-system.md) §3.3 logic).
...
- **Multiple CMs (rare):** if a campaign has more than one user with the `cm` role, they have full campaign authority each. Co-approval between them is configurable per kind ([PRD-5](/prds/prd-5-approval-system.md) §3.2). This is **distinct** from Crusade Team Leaders, who are scoped to a team.
...
1. **Pending approvals count** (clickable → [PRD-5](/prds/prd-5-approval-system.md) inbox)
...
- For each `ApprovalRequest`:
  ...
  - Rule check results ([PRD-3](/prds/prd-3-army-export-versioning.md)): pass/warn/fail with details
...
- When the crusade is archived (`Campaign.status = 'archived'`), all inboxes become read-only and every player across teams sees the full history (per [PRD-0](/prds/prd-0-overview.md) §3b post-crusade relaxation).
...
- **Teams** — add/rename/delete/reorder/color teams; manage team leaders per team (per [PRD-1](/prds/prd-1-crusade-master-admin.md) §4.2)
...
- Activating a phase emits `campaign.phase_activated` event ([PRD-4](/prds/prd-4-events-deltas.md) §3).
...
**Why no system enforcement (v3.19):** the user explicitly clarified that phase effects are cosmetic and players enforce the rules themselves. The app stays out of the way. The `effects` field is reserved in the data model for v1.x+ (a future version could add structured effects that the system enforces — e.g., "during Phase 1, drop-pod units are unavailable"). The [PRD-0](/prds/prd-0-overview.md) §4b.2 principle (NR-as-source-of-truth; app is not a rules adjudicator) extends to phases.
...
**Active rule packs:** which `RulePack`s ([PRD-3](/prds/prd-3-army-export-versioning.md) §6) are enabled for this campaign. The CM toggles these on/off in the Rules section.

**Save behavior — ruleset-change warning (v3.17: always fires):**

When the CM clicks "Save" on the Approvals section OR the Rules section, the warning modal **always** fires (per [PRD-5](/prds/prd-5-approval-system.md) §5.4). The modal shows:
...
Per user (v3.17): even when no pending approvals are affected, the modal still opens — the CM should always see the impact assessment before saving. Variant B in [PRD-5](/prds/prd-5-approval-system.md) §5.4 shows the zero-affected case.
...
Deletable: archive (soft) or hard-delete (typed confirmation required). Archive is the post-crusade state where all data becomes read-only-visible to all players across teams (per [PRD-0](/prds/prd-0-overview.md) §3b data-isolation relaxation).
...
- **Bulk-approve routine battle updates**: a checkbox per row; "Approve 4 selected" is one click. Per the auto-approve rules in [PRD-5](/prds/prd-5-approval-system.md), the bulk action refuses if any selected item is non-routine (a failed OoA, a requisition, etc.). Mike gets a clear refusal: "2 items skipped (manual review needed): see details."
...
**Edge case:** Mike opens a roster, scrolls around, and a player uploads a new version of the same roster while he's reviewing. Drift detection ([PRD-5](/prds/prd-5-approval-system.md) §6) flags it; Mike sees "Drift detected — player uploaded new version 2 minutes ago. Approve original or re-validate?" The original approval is the older draft; Mike chooses to wait for the player to re-submit the new one.
...
## 7. Out of Scope ([PRD-1](/prds/prd-1-crusade-master-admin.md))
...
- **[PRD-0](/prds/prd-0-overview.md)**: `Tenant`, `User`, `Campaign`, `CampaignMember`, `CrusadeSupplement`
- **[PRD-5](/prds/prd-5-approval-system.md)**: approval inbox link
- **[PRD-3](/prds/prd-3-army-export-versioning.md)**: roster approval status surfaces in dashboard
- **[PRD-4](/prds/prd-4-events-deltas.md)**: event feed surfaces in dashboard
