...
**Hard rule**: a player can only file a battle update, requisition, or any other event ([PRD-4](/prds/prd-4-events-deltas.md)) if the most recent state of their Roster is `RosterApproved` at the relevant timestamp.
...
**Scope of rule engine — campaign-level only (per [PRD-0](/prds/prd-0-overview.md) §4b.2):**
...
All built-in rules are **campaign-level only** (per [PRD-3](/prds/prd-3-army-export-versioning.md) §6 + [PRD-0](/prds/prd-0-overview.md) §4b.2). The v3.21 audit removed three unit-level rules that violated the principle (see §6.5).
...
**Storage** ([PRD-0](/prds/prd-0-overview.md) shared model):
```ts
RuleDefinition {
  id, tenantId,
...
- [PRD-1](/prds/prd-1-crusade-master-admin.md) (CM dashboard): new "Rules" tab in Campaign Settings
...
Per v3.21 cross-PRD audit: three rules from the v1 ship list violated the campaign-level-only principle ([PRD-0](/prds/prd-0-overview.md) §4b.2, [PRD-3](/prds/prd-3-army-export-versioning.md) §6). These rules were removed:
...
- **`honour-provenance`** (was: warn) — checked if a new honour in the draft was earned via prior approved event. Unit-level (which honours are legal for which units) and assumes the app tracks unit-level state — which it doesn't per [PRD-0](/prds/prd-0-overview.md) §4b.2. Removed.
...
When the player submits the draft (status: `pending_review` → `pending_approval`), it enters the CM's approval inbox ([PRD-5](/prds/prd-5-approval-system.md)).
...
A CM can roll back a RosterApproval within a configurable window (default 7 days). Rollback inverts the deltas that the rolled-back Roster introduced. Since the Timeline ([PRD-4](/prds/prd-4-events-deltas.md)) is the source of truth, the rollback is just a new event in the timeline that says "RosterApproved X is superseded by Y."
...
- Manual unit editing in the UI (JSON import is the canonical path; CMs have an override tool per [PRD-1](/prds/prd-1-crusade-master-admin.md))
...
- **[PRD-0](/prds/prd-0-overview.md)**: `Roster`, `RosterDraft`, `RosterApproved`, `CrusadeForceState`, `RuleDefinition`, `RuleCheck`
- **[PRD-4](/prds/prd-4-events-deltas.md)**: every approval creates an event
- **[PRD-5](/prds/prd-5-approval-system.md)**: approval workflow consumes rule-check results
...
5. **Player imports during a pending approval**: the new draft is staged separately; pending approval refers to the older draft. Drift handled by [PRD-5](/prds/prd-5-approval-system.md).
...
- [Rule Engine](/references/rule-engine.md)
- [Wahapedia](/references/wahapedia.md)
- [bs-roster-parser](/references/bs-roster-parser.md)
